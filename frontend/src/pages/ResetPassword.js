  ...
  return (
    <div className={`relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 ${isDark ? 'bg-[radial-gradient(circle_at_top,_#1e293b,_#111827_35%,_#020617_80%)]' : 'bg-[radial-gradient(circle_at_top,_#dbeafe,_#93c5fd_35%,_#2563eb_75%,_#1e3a8a_100%)]'}`}>
      <div className={`w-full max-w-lg rounded-[2rem] border p-8 backdrop-blur-xl ${isDark ? 'border-slate-700/80 bg-slate-900/75 shadow-[0_30px_80px_rgba(2,6,23,0.7)]' : 'border-white/70 bg-white/90 shadow-[0_30px_80px_rgba(30,64,175,0.28)]'}`}>
        ...
        <h1 className={`mt-7 text-3xl font-bold tracking-tight ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Set a new password</h1>
        <p className={`mt-3 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Choose a new password to continue.</p>
